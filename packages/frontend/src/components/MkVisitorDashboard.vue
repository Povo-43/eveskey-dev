	...
	<div :class="[$style.main, $style.panel]">
		...
		<div :class="$style.mainFg">
			...
			<div class="_gaps_s" :class="$style.mainActions">
				<MkButton :class="$style.mainAction" full rounded gradate data-cy-signup style="margin-right: 12px;" @click="signup()">EvexAccountで作成</MkButton>
				<MkButton :class="$style.mainAction" full rounded link to="https://misskey-hub.net/servers/">{{ i18n.ts.exploreOtherServers }}</MkButton>
				<MkButton :class="$style.mainAction" full rounded data-cy-signin @click="signin()">EvexAccountで続行</MkButton>
			</div>
		</div>
	</div>
